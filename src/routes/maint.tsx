  ...
  return (
    <div className="engine-page">
      <SwissSubpageHero
        ...
        title={<span>Zero ops.<br />Just upgrade.</span>}
      ...
      <main className="engine-main">
        ...
        <section className="engine-section">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1px", background: "var(--border)", border: "1px solid var(--border)" }}>
            {/* Legacy */}
            <div style={{ background: "var(--background)", padding: "2.5rem", display: "flex", flexDirection: "column", gap: "0" }}>
              <div style={{ fontFamily: "var(--font-mono)", fontSize: "0.6rem", color: "var(--steel)", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: "1.5rem" }}>Legacy Weekly Ops</div>
              {LEGACY_OPS.map((item, i) => (
                <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 60px", gap: "1rem", padding: "0.75rem 0", borderBottom: i < LEGACY_OPS.length - 1 ? "1px solid var(--border)" : "none", alignItems: "center" }}>
                  <div style={{ display: "flex", gap: "0.6rem", fontFamily: "var(--font-sans)", fontSize: "0.8rem", color: "var(--muted)", lineHeight: 1.4 }}>
                    <span style={{ color: "#ff3b30", fontFamily: "var(--font-mono)", fontWeight: 700, flexShrink: 0 }}>✗</span>
                    {item.task}
                  </div>
                  <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.65rem", color: "#ff3b30", textAlign: "right" }}>{item.time}</span>
                </div>
              ))}
              <div style={{ marginTop: "2rem", fontFamily: "var(--font-display)", fontSize: "1.4rem", fontWeight: 800, letterSpacing: "-0.04em", color: "#ff3b30" }}>~4h / week</div>
            </div>

            {/* VantaDB */}
            <div style={{ background: "var(--surface-raised)", padding: "2.5rem", borderLeft: "2px solid var(--amber)", display: "flex", flexDirection: "column", gap: "0" }}>
              <div style={{ fontFamily: "var(--font-mono)", fontSize: "0.6rem", color: "var(--amber)", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: "1.5rem" }}>VantaDB Weekly Ops</div>
              {VANTA_OPS.map((item, i) => (
                <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 60px", gap: "1rem", padding: "0.75rem 0", borderBottom: i < VANTA_OPS.length - 1 ? "1px solid var(--border)" : "none", alignItems: "center" }}>
                  <div style={{ display: "flex", gap: "0.6rem", fontFamily: "var(--font-sans)", fontSize: "0.8rem", color: "var(--foreground)", lineHeight: 1.4 }}>
                    <span style={{ color: "var(--amber)", fontFamily: "var(--font-mono)", fontWeight: 700, flexShrink: 0 }}>✓</span>
                    {item.task}
                  </div>
                  <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.65rem", color: "var(--amber)", textAlign: "right" }}>{item.time}</span>
                </div>
              ))}
              <div style={{ marginTop: "2rem", fontFamily: "var(--font-display)", fontSize: "1.4rem", fontWeight: 800, letterSpacing: "-0.04em", color: "var(--amber)" }}>~30s / week</div>
            </div>
          </div>
          ...
          <div style={{ border: "1px solid var(--border)", borderTop: "none", background: "var(--surface)", padding: "1.5rem 2.5rem", display: "grid", gridTemplateColumns: "140px 1fr", gap: "2rem", alignItems: "center" }}>
            <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.6rem", color: "var(--amber)", textTransform: "uppercase", letterSpacing: "0.08em" }}>KEY INSIGHT</span>
            <p style={{ fontFamily: "var(--font-sans)", fontSize: "0.82rem", color: "var(--muted)", lineHeight: 1.7, margin: 0 }}>
              Because VantaDB runs as an embedded library — not a separate server — there's nothing to deploy, monitor, or scale independently. Your application's lifecycle <em>is</em> the database lifecycle. No pager duty. No 2 AM wakeups.
            </p>
          </div>
        </section>
      </main>
