...
function tokenizePython(line: string): Token[] {
  ...
  let i = 0;
  while (i < line.length) {
    if (line[i] === "#") { parts.push({ t: "comment", v: line.slice(i) }); break; }
    if (line[i] === '"' || line[i] === "'") {
      const q = line[i];
      let j = i + 1;
    ...
    if (/[\[\](){},.]/.test(line[i])) { parts.push({ t: "punctuation", v: line[i] }); i++; continue; }
    if (/[=+\-*/<>!&|%^~?:@]/.test(line[i])) { parts.push({ t: "operator", v: line[i] }); i++; continue; }
...
function tokenizeRust(line: string): Token[] {
  ...
  let i = 0;
  while (i < line.length) {
    if (line[i] === "/" && line[i + 1] === "/") { parts.push({ t: "comment", v: line.slice(i) }); break; }
    if (line[i] === '"') {
      let j = i + 1;
...
export function liveHighlight(code: string, typed: number, lang: string, cursor: boolean): JSX.Element[] {
