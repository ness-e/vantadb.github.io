...
export default class PostProcess {
  ...
  // Textures
  _setTextures() {
    const displacementTexture = (this.displacementTexture =
    ...
    const displacementTextureNode = (this.displacementTextureNode = texture(
      ...
      uv()
        .mul(this.transitionPassParams.uvScale)
  ...
  setDisplacementPass(nodeA, nodeB) {
    ...
    this.composer.outputNode = Fn((params) => {
      ...
      const displacement1 = getRotM(angle1).mul(
        dispVec
          .mul(this.uniforms.displacementPassParams.intensity)
          .mul(this.uniforms.displacementPassParams.progress),
      );
      const distortedPosition1 = screenUV.sub(displacement1.yx);
      const t1 = scenePassPreloadTexture.sample(distortedPosition1).toVar();

      const displacement2 = getRotM(angle2)
        .mul(
          dispVec
            .mul(this.uniforms.displacementPassParams.intensity)
            .mul(float(1.0).sub(this.uniforms.displacementPassParams.progress)),
        )
        .toVar();
  ...
  _calculateUVTransform() {
    const { uvScale, uvOffset } = Helpers.calculateUVTransform(
      this.displacementTexture,
      this.sizes,
    );
