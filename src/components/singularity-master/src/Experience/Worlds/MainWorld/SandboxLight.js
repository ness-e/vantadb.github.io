...
export default class SandboxLight extends Model {
  ...
  setModel() {
    ...
    const allPositions = new Float32Array(
      floorGeometry.attributes.position.array.length +
        sphereGeometry.attributes.position.array.length,
    );
    ...
    this.computePositions = Fn(() => {
      ...
      If(dotProduct.lessThan(angleLimit), () => {
        ...
        pos.assign(
          posInit.add(simplexNoise4d(vec4(posInit.mul(uFlowFieldFrequency).add(1.0), time))),
        );
      ...
      floorGeometry.attributes.position.count + sphereGeometry.attributes.position.count,
    );
