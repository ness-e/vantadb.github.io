  ...
  setModel() {
    ...
    material.colorNode = Fn(() => {
      ...
      Loop(iterCount, ({ i }) => {
        ...
        // Outer glow: fills the dark edge of the ramp (pos 1.0 = black) with warm glow
        const outerGlow = smoothstep(0.4, 0.8, xyLen);
        const outerColor = vec3(0.6, 0.25, 0.08).mul(outerGlow).mul(this.uniforms.rampEmission);
        const emissiveCol = baseCol
          .mul(this.uniforms.rampEmission)
          .add(this.uniforms.emissionColor)
          .add(outerColor);

        // Core suppression — smooth gradient from black (center) to emissive (outward)
        const rLenNow = lengthSqrt(rayPos);
        const coreDark = smoothRange(rLenNow, 0.0, originRadius.mul(2), 1.0, 0.0);
        const shadedCol = mix(emissiveCol, vec3(0), coreDark);

        // Alpha shaping
        const zAbs = abs(rayPos.z);
        const aNoise = noiseAmpLen.sub(0.75).mul(-0.6);
        const aPre = zAbs.add(aNoise);
        const aRadial = float(1).sub(smoothstep(0.2, 1.0, xyLen));
        const aBand = smoothRange(aPre, bandWidth, 0, 0, aRadial);
        const coreAlpha = smoothRange(rLenNow, 0.0, originRadius, 1.0, 0.0);
        const alphaLocal = mix(aBand, 1.0, coreAlpha);
