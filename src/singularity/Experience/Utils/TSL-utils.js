...
const rotateY = /*#__PURE__*/ Fn(([v_immutable, angle_immutable]) => {
  ...
  return vec3(v.x.mul(c).add(v.z.mul(s)), v.y, v.x.mul(s).negate().add(v.z.mul(c)));
}).setLayout({
  name: "rotateY_fast",
  type: "vec3",
  inputs: [
...
const directionalBlur = /*#__PURE__*/ Fn(
  ([uv_immutable, direction_immutable, radius_immutable]) => {
    ...
    Loop(
      { start: int(radius.negate()), end: int(radius), type: "int", condition: "<=" },
      ({ i }) => {
...
const bezier = /*#__PURE__*/ Fn(
  ([P0_immutable, P1_immutable, P2_immutable, P3_immutable, t_immutable]) => {
    const t = float(t_immutable).toVar();
    ...
    return sub(1.0, t)
      ...
      .mul(sub(1.0, t))
      .mul(P0)
      .add(mul(3.0, sub(1.0, t)).mul(sub(1.0, t)).mul(t).mul(P1))
      .add(mul(3.0, sub(1.0, t)).mul(t).mul(t).mul(P2))
      .add(t.mul(t).mul(t).mul(P3));
  },
).setLayout({
  name: "bezier",
  type: "vec3",
...
const bezierGrad = /*#__PURE__*/ Fn(
  ([P0_immutable, P1_immutable, P2_immutable, P3_immutable, t_immutable]) => {
    ...
    return mul(3.0, sub(1.0, t))
      .mul(sub(1.0, t))
      .mul(P1.sub(P0))
      .add(mul(6.0, sub(1.0, t)).mul(t).mul(P2.sub(P1)))
      .add(mul(3.0, t).mul(t).mul(P3.sub(P2)));
  },
).setLayout({
  name: "bezierGrad",
  type: "vec3",
...
const noise = /*#__PURE__*/ Fn(([p_immutable]) => {
  ...
  return mix(
    mix(
      mix(
        dot(_hash(i.add(vec3(0.0, 0.0, 0.0))), f.sub(vec3(0.0, 0.0, 0.0))),
        dot(_hash(i.add(vec3(1.0, 0.0, 0.0))), f.sub(vec3(1.0, 0.0, 0.0))),
        u.x,
      ),
      mix(
        dot(_hash(i.add(vec3(0.0, 1.0, 0.0))), f.sub(vec3(0.0, 1.0, 0.0))),
        dot(_hash(i.add(vec3(1.0, 1.0, 0.0))), f.sub(vec3(1.0, 1.0, 0.0))),
        u.x,
      ),
      u.y,
    ),
    mix(
      mix(
        dot(_hash(i.add(vec3(0.0, 0.0, 1.0))), f.sub(vec3(0.0, 0.0, 1.0))),
        dot(_hash(i.add(vec3(1.0, 0.0, 1.0))), f.sub(vec3(1.0, 0.0, 1.0))),
        u.x,
      ),
      mix(
        dot(_hash(i.add(vec3(0.0, 1.0, 1.0))), f.sub(vec3(0.0, 1.0, 1.0))),
        dot(_hash(i.add(vec3(1.0, 1.0, 1.0))), f.sub(vec3(1.0, 1.0, 1.0))),
        u.x,
      ),
      u.y,
    ),
...
const terrainHeight = /*#__PURE__*/ Fn(([worldPos_immutable]) => {
  ...
  name: "terrainHeight",
  type: "vec3",
  inputs: [{ name: "worldPos", type: "vec3" }],
});

const lambertLight = /*#__PURE__*/ Fn(
  ([normal_immutable, viewDir_immutable, lightDir_immutable, lightColour_immutable]) => {
    ...
    const backlight = float(
      saturate(dot(viewDir, lightDir.negate()).add(wrap).div(add(1.0, wrap))),
    ).toVar();
...
const hemiLight = /*#__PURE__*/ Fn(
  ([normal_immutable, groundColour_immutable, skyColour_immutable]) => {
...
const phongSpecular = /*#__PURE__*/ Fn(
  ([normal_immutable, lightDir_immutable, viewDir_immutable]) => {
    const viewDir = vec3(viewDir_immutable).toVar();
    const lightDir = vec3(lightDir_immutable).toVar();
    const normal = vec3(normal_immutable).toVar();
...
const ColorRamp3_Linear = /*#__PURE__*/ Fn(
  ([T_immutable, A_immutable, B_immutable, C_immutable]) => {
...
const ColorRamp2_Constant = /*@__PURE__*/ Fn(
  ([T, A, B]) => {
    If(T.lessThan(B.w), () => {
      return A.xyz;
    });
...
const ColorRamp3_Constant = /*@__PURE__*/ Fn(
  ([T, A, B, C]) => {
    If(T.lessThan(B.w), () => {
      return A.xyz;
    });
...
const ColorRamp4_Constant = /*@__PURE__*/ Fn(
  ([T, A, B, C, D]) => {
    If(T.lessThan(B.w), () => {
      return A.xyz;
    });
...
const grid = /*#__PURE__*/ Fn(([uv_immutable, lineWidth_immutable]) => {
  ...
  const targetWidth = vec2(
    select(invertLineX, sub(1.0, lineWidth.x), lineWidth.x),
    select(invertLineY, sub(1.0, lineWidth.y), lineWidth.y),
  ).toVar();
...
const linearToSrgb = Fn(([lin]) => {
  ...
  const high = pow(lin, vec3(1.0 / 2.4))
    .mul(1.055)
    .sub(0.055);
  return mix(low, high, step(0.0031308, lin));
});

const noise21 = /*@__PURE__*/ Fn(
  ([p]) => {
    const i = floor(p);
    const f = fract(p);
    ...
    const n = mix(
      mix(
        dot(float(-1.0).add(mul(2.0, _hash(i.add(vec2(0.0))))), f.sub(vec2(0.0))),
        dot(float(-1.0).add(mul(2.0, _hash(i.add(vec2(1.0, 0.0))))), f.sub(vec2(1.0, 0.0))),
        u.x,
      ),
      mix(
        dot(float(-1.0).add(mul(2.0, _hash(i.add(vec2(0.0, 1.0))))), f.sub(vec2(0.0, 1.0))),
        dot(float(-1.0).add(mul(2.0, _hash(i.add(vec2(1.0, 1.0))))), f.sub(vec2(1.0, 1.0))),
        u.x,
      ),
      u.y,
    );
...
const adjustSaturation = /*@__PURE__*/ Fn(
  ([color, saturation]) => {
...
const adjustTemperature = /*@__PURE__*/ Fn(
  ([color, temp]) => {
...
const adjustLevels = /*@__PURE__*/ Fn(
  ([color, blackLevel, whiteLevel]) => {
...
const brickTexture = Fn(
  ({
    ...
    const st = vec2(st0.x.mul(scale).div(brickWidth), st0.y.mul(scale).div(rowHeight));
...
const ColorRamp4_BSpline = /*@__PURE__*/ Fn(
  ([T, A, B, C, D]) => {
...
const ColorRamp3_BSpline = /*@__PURE__*/ Fn(
  ([T, A, B, C]) => {
    const AB = B.w.sub(A.w);
    const BC = C.w.sub(B.w);
